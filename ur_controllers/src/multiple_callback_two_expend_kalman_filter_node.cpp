...
public:
        ...
        : Node("expend_kalman_filter_two")  // 设置节点名称
    {
        ...
        // 创建订阅者，订阅 "/callback12_expend_kalman_filter" 话题 
        callback12_expend_kalman_filter_sub = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            "/callback12_expend_kalman_filter", 10,  // 话题名称和队列大小
            std::bind(&MultipleCallbackTwoExpendKalmanFilterSubscriber::callback12_expend_kalman_filter_callback, this, std::placeholders::_1));
...
private:
    ...
    void callback12_expend_kalman_filter_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        d2theta_prior << msg->data[0], msg->data[1], msg->data[2], msg->data[3], msg->data[4], msg->data[5];
        if(msg->data[6] - now_time < 0.0001 && msg->data[6] - now_time > -0.0001) {
            result_12 = 1;
        }
    }
    ...
    void timer_callback() {
        ...
        if (result_3 * result_4 * result_5 * result_6 * result_7 * result_8 * result_9 * result_10 * result_11 * result_12 > 0) {
            whether_send = true;
            result_3 = 0;
            result_4 = 0;
            result_5 = 0;
        ...
        if (whether_send) {
            ...
            for (int i = 1; i <= 6; i++) {
                ...
                msg_send.data[78 + i] = d2theta_prior(i-1, 0);
            }
            for (int i = 1; i <= 7; i++) {
                msg_send.data[72 + i-1] = A(12, i-1);
            }
            msg_send.data[85] = now_time;
    ...
    rclcpp::Subscription<std_msgs::msg::Float64MultiArray>::SharedPtr callback12_expend_kalman_filter_sub;
