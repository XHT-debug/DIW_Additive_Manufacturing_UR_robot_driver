...
Eigen::MatrixXd last_Ad_callback = Eigen::MatrixXd::Zero(12, 12);
...
// 简化卡尔曼滤波计算参数
Eigen::MatrixXd d2theta_prior = Eigen::MatrixXd::Zero(6, 1);
...
void calculateExpendKalmanFilter_prior(const Eigen::MatrixXd& torque_k_1, const double& dt)
{
    ...
    {
        X_k_prior.block<6, 1>(6, 0) = expend_kalman_filter_result_k_1.X_posterior.block<6, 1>(6, 0) + d2theta_prior * dt;
    }
...
private:
    ...
    void joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg) {
        ...
        if (start_expend_kalman_filter && whether_torque_correction) {
            ...
            // RCLCPP_INFO(this->get_logger(), "expend_kalman_filter_result_k_1_msg time = %ld microseconds", duration.count());
            ...
            auto duration_1 = std::chrono::duration_cast<std::chrono::microseconds>(end_time_1 - end_time);
            RCLCPP_INFO(this->get_logger(), "calculateExpendKalmanFilter_prior time = %ld microseconds", duration_1.count());

            if (first_time == false)
            {
                calculateExpendKalmanFilter_posterior(last_Ad_callback, dt_outside);
                ...
                // RCLCPP_INFO(this->get_logger(), "calculateExpendKalmanFilter_posterior time = %ld microseconds", duration_2.count());
            }
    ...
    void callback2_expend_kalman_filter_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        ...
        last_Ad_callback = Eigen::MatrixXd::Identity(12, 12) + A_callback * dt_outside;
        ...
        d2theta_prior << msg->data[79], msg->data[80], msg->data[81], msg->data[82], msg->data[83], msg->data[84];
