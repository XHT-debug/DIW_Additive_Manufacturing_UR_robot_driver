...
class ExpendKalmanFilterTestSubscriber : public rclcpp::Node
{
public:
    ExpendKalmanFilterTestSubscriber()
        : Node("expend_kalman_filter_test")  // 设置节点名称
    {
        // 创建订阅者，订阅 "/joint_states" 话题
        joint_states_sub = this->create_subscription<sensor_msgs::msg::JointState>(
            "/joint_states", 10,  // 话题名称和队列大小
            std::bind(&ExpendKalmanFilterTestSubscriber::joint_state_callback, this, std::placeholders::_1));
        ...
        start_expend_kalman_filter_sub = this->create_subscription<std_msgs::msg::Bool>(
            ...
            std::bind(&ExpendKalmanFilterTestSubscriber::start_expend_kalman_filter_callback, this, std::placeholders::_1));

        // 创建订阅者，订阅 "/start_debug" 话题 
        start_debug_sub = this->create_subscription<std_msgs::msg::Bool>(
            "/start_debug", 10,  // 话题名称和队列大小
            std::bind(&ExpendKalmanFilterTestSubscriber::start_debug_callback, this, std::placeholders::_1));

        theta_from_expend_kalman_filter_pub = this->create_publisher<geometry_msgs::msg::Twist>("/theta_from_expend_kalman_filter_test", 10);
        theta_from_joint_position_with_noise_pub = this->create_publisher<geometry_msgs::msg::Twist>("/theta_from_joint_position_with_noise_test", 10);
        theta_from_joint_position_pub = this->create_publisher<geometry_msgs::msg::Twist>("/theta_from_joint_position_test", 10);
    }
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<ExpendKalmanFilterTestSubscriber>();  // 创建订阅者节点
